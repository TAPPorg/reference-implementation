...
void mpiBroadcastC64(std::complex<double>& num) {
  int rootRank = 0;
  MPI_Bcast(&num, 2, MPI_DOUBLE, rootRank, MPI_COMM_WORLD);
}
...
int waitWorkersFinished(){
  ...
  if(rank == rootRank){
    ...
    std::cout << "Master: All workers have finished." << std::endl;
  }
  else{
...
int getLocalDataSizes(std::vector<int64_t>& data_size_){
  int rootRank = 0;
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  ...
  if(rank == rootRank){
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &np);
    std::vector<int> finished_flags(np - 1, 0); // Track worker completion
    std::vector<int64_t> data_size(np, 0); // data size stored at worker
    MPI_Status status;
    for(int worker=1; worker<np; worker++) { // Loop until all workers finish
      int64_t finished;
      MPI_Recv(&finished, 1, MPI_INT64_T, worker, 0, MPI_COMM_WORLD, &status);
      int worker_rank = status.MPI_SOURCE;
 
      if (finished_flags[worker_rank - 1] == 0) {
        data_size[worker_rank] = finished;
        std::cout << "Master: Worker " << worker_rank << " has sent size:" << data_size[worker_rank] << std::endl;
        finished_flags[worker_rank - 1] = 1; // Mark worker as finished
      }
      else{
        std::cout << "Master: Error: too many messages from worker " << worker_rank << " msg: " << finished << std::endl;
        exit(404);
      }
    }
    data_size_ = data_size;
    std::cout << "Master: All workers have sent size." << std::endl;
  }
  else{
      int64_t finished = data_size_[rank]; // Signal completion
      MPI_Send(&finished, 1, MPI_INT64_T, rootRank, 0, MPI_COMM_WORLD);
      std::cout << "Worker " << rank << ": sent local data size to master: " << finished << std::endl;
  }
...
int distributeArrayDataPart2(void* full_data, int64_t local_numel, int64_t* local_idx, void* local_data, int& data_type){
  ...
  if(rank == rootRank){
    ...
    for(int worker=1; worker<np; worker++) { // Loop until all workers finish
      ...
      if(numel > 0){
        local_idx = new int64_t[numel];
        MPI_Recv(local_idx, numel, MPI_INT64_T, worker, 0, MPI_COMM_WORLD, &status);
        ...
        if(data_type == 1){ 
          ...
          for(int64_t k=0;k<numel;k++){
            local_data_[k] = full_data_[local_idx[k]]; 
          }
          ...
          MPI_Send(local_data_, 2*numel, MPI_DOUBLE, worker, 0, MPI_COMM_WORLD);
          delete[] local_data_;
        }
        else{ 
    ...
    if(local_numel > 0){
      ...
      if(data_type == 1){ 
        ...
        MPI_Recv(local_data_, 2*local_numel, MPI_DOUBLE, rootRank, 0, MPI_COMM_WORLD, &status);
      }
    }
  }
