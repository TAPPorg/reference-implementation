...
#define TAPP_EXECUTE_PRODUCT_TEMPLATE2(T_A, T_B, T_C, T_D) Tapp_execute_product_ ## T_A ## _ ## T_B ## _ ## T_C ## _ ## T_D
#define TAPP_EXECUTE_PRODUCT_TEMPLATE(T_A, T_B, T_C, T_D) TAPP_EXECUTE_PRODUCT_TEMPLATE2(T_A, T_B, T_C, T_D)
...
// Declares one Tapp_execute_product_<T_A>_<T_B>_<T_C>_<T_D> per (type_A, type_B, type_C,
// type_D) combination, plus TAPP_PRODUCT_DISPATCH[type_A][type_B][type_C][type_D] mapping
// to them. Generated by generate_product_instantiations.py - see that file to regenerate.
                                ...
                                void* D)
{
    ...
    // TAPP_datatype values >= 0x1000 are reserved for implementation-defined types (see
    // datatype.h) and any other out-of-range value is simply invalid input; neither has a
    // slot in TAPP_PRODUCT_DISPATCH, so they must be rejected before indexing into it.
    if (plan_ptr->type_A < 0 || plan_ptr->type_A >= TAPP_PRODUCT_NUM_DATATYPES ||
        plan_ptr->type_B < 0 || plan_ptr->type_B >= TAPP_PRODUCT_NUM_DATATYPES ||
        plan_ptr->type_C < 0 || plan_ptr->type_C >= TAPP_PRODUCT_NUM_DATATYPES ||
        plan_ptr->type_D < 0 || plan_ptr->type_D >= TAPP_PRODUCT_NUM_DATATYPES)
    {
        return 34;
    }

    TAPP_execute_product_fn fn =
        TAPP_PRODUCT_DISPATCH[plan_ptr->type_A][plan_ptr->type_B][plan_ptr->type_C][plan_ptr->type_D];
    if (fn == NULL)
    {
        return 34;
    }

    return fn(plan, exec, status, alpha, A, B, beta, C, D);
}
