                                ...
                                void* D)
{
    ...
    if((*exec_int_ptr) == 1 || (*exec_int_ptr) == 12 ) { // 1 = bruteforce, 2 = tblis, 12 = tblis + bruteforce check
        ...
        for (int64_t h = 0; h < plan_ptr->H_nmode; h++)
        {
            ...
            for (int64_t fa = 0; fa < plan_ptr->FA_size; fa++)
            {
                ...
                for (int64_t fb = 0; fb < plan_ptr->FB_size; fb++)
                {
                    ...
                    for (int64_t b = 0; b < plan_ptr->B_nmode; b++)
                    {
                        int64_t B_offset = calcualte_offset(B_coords, plan_ptr->B_nmode, plan_ptr->B_strides);
                        int64_t offset_D = H_offset_D + FA_offset_D + FB_offset_D + B_offset;
