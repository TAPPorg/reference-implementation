...
void contraction_PPL(const int nocc, const int nvirt)
{
    ...
    int64_t strides_A[4] = {1, nvirt, nvirt*nvirt, nvirt*nvirt*nvirt};
    ...
    int64_t A_nelem = nvirt*nvirt*nvirt*nvirt;
    int64_t B_nelem = nvirt*nvirt*nocc*nocc;
    int64_t C_nelem = nvirt*nvirt*nocc*nocc;
    int64_t D_nelem = nvirt*nvirt*nocc*nocc;
    ...
    int threads = 256, blocks = 1024; // Fixed typo "nt" -> "int"
    ...
    std::cout << "Total time for " << iterations << " iterations: " << milliseconds << " ms\n";
    std::cout << "Average time per iteration: " << milliseconds / iterations << " ms\n";
